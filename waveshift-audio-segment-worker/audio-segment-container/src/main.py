...
"""
音频切分容器服务 - 重构版
专注于纯 FFmpeg 音频处理，移除业务逻辑和 I/O 操作
支持两种模式：
1. /process-single - 新的简化接口（推荐）
2. /segment - 保留的完整接口（兼容性）
"""
import os
import asyncio
import logging
...
async def process_single_segment(request: Request):
    """
    🎯 新接口：处理单个音频片段 - 纯 FFmpeg 计算服务
    输入：音频二进制数据 + 时间范围 + 参数（通过请求头）
    输出：处理后的音频二进制数据
    
    专注纯计算，无 R2 操作，无业务逻辑，供 Worker 调用
    """
    try:
        # 从请求头获取参数
        ...
        if not time_ranges_str:
            return {"success": False, "error": "Missing X-Time-Ranges header"}
        ...
        return {"success": False, "error": f"Invalid time ranges format: {e}"}
    except Exception as e:
